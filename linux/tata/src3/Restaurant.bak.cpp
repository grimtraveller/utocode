...
bool Restaurant::get(istream&get,bool file)
{
	...
	if ( file)
	{
		if ( !(get>>ownername>>name>>address>>telephone>>mealcount) ) return false;
...
void Restaurant::put(ostream&put,bool file)
{
	if (file)
	{
		put<<"R"<<" "<<ownername<<" "<<name<<" "<<address<<" "<<telephone<<" "<<mealcount<<" ";
	...
	put<<"Restaurant No."<<number<<" Owner name:"<<ownername<<" Name:"<<name
